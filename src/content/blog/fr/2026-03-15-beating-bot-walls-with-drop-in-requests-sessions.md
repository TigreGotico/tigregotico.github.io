...
- **« Qu'êtes-vous ? »** Cloudflare et consorts signalent les requêtes non pas pour
  *ce* que vous demandez, mais pour *l'allure* que vous avez sur le fil : votre
  handshake TLS, votre empreinte JA3 (un hachage de la façon dont votre handshake TLS
  est construit, qui diffère entre un navigateur et une bibliothèque HTTP ordinaire
  même quand les deux demandent la même page), votre capacité à exécuter un défi
  JavaScript. Un handshake `requests` ordinaire ne ressemble en rien à celui d'un
  navigateur, il se fait donc repérer par des vérifications visant l'abus scripté,
  même quand le trafic lui-même est bénin.
...
sortie) reste cohérent de bout en bout, ce qui est le comportement correct
