---
title: "組み合わせ可能でそのまま差し替えられる requests セッションでボット対策の壁を突破する"
description: "ホットパスでヘッドレスブラウザを起動することなく、公開データへの回復力ある(レジリエントな)アクセスを維持する方法: TLS フィンガープリントのなりすまし、JS チャレンジ向けの FlareSolverr プロキシ、Wayback Machine へのフォールバック、そして IP ローテーション — これらすべてを、組み合わせ可能な 2 つの requests.Session サブクラス、unblock_requests と anon_requests の背後に収めています。"
date: 2026-03-15
lang: ja
...
問題となるのはたいていデータそのものではなく、その手前に立ちはだかる壁です。
そしてその壁は、2 つの別々の問いを投げかけてきます。

- **「お前は何者だ?」** — Cloudflare やその類は、あなたが*何を*要求するかではなく、
  通信路上でどう*見えるか*でリクエストをブロックします。つまり TLS ハンドシェイク、
  JA3 フィンガープリント、JavaScript チャレンジを実行できるかどうか、です。
- **「お前は誰だ?」** — IP レピュテーションやレート制限はフィンガープリントを完全に
  無視し、1 つのアドレスからどれだけのリクエストが来るかを数えます。

この 2 つの問いは直交しているため、私たちはきれいに積み重ねられる 2 つの小さな
...
`unblock_requests` は**ボット検出**に対して防御します。トランスポートは
`mode=` キーワード引数(または環境変数 `UNBLOCK_REQUESTS_TRANSPORT` — 明示的な
キーワード引数が常に優先されます)で選びます。主要な 4 つは次のとおりです。

| モード | 何をするか |
|---|---|
| `curl_cffi` *(デフォルト)* | `curl_cffi` による Chrome の TLS/JA3 なりすまし。追加のインフラなしで、ほとんどのネットワークでボットチェックを通過します。 |
...
直交するもう 1 つの問題が **IP レピュテーション**です。完璧なフィンガープリントで
あっても、すべてのリクエストが 1 つのアドレスから来れば、レート制限を受けるか
BAN されます。`anon_requests` はこれを `RotatingProxySession`(スクレイプした
公開プロキシ、任意のバリデーション、SOCKS5/HTTP)と `RotatingTorSession`
(ローテーションする Tor 回路)で処理します。各リクエストは新しい出口を通って
外に出て行き、死んだプロキシは接続失敗時にローテーションで外されます。
...
ヘッドレスブラウザを駆動します。つまり、チャレンジを解決する IP は、リクエストの
残りの部分が使うのと同じローテーションされた IP です。防御側が気づけるような
フィンガープリントと出口ノードの分離はありません。
