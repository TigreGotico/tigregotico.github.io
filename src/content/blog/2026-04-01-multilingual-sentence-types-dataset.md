...
The dataset uses a flat set of six labels: one `label` column per row, with no type/sub-type split. These labels map directly to how `little_questions` (the inference library that consumes this data) routes utterances:

- **wh_question**: questions built around a wh-word (what, where, who, and so on).
- **polar_question**: yes/no questions. The EAT (Expected Answer Type) taxonomy inside `little_questions` adds 53 fine-grained answer-type labels (person, location, quantity, definition, and so on) on top of the question labels, but sentence-type classification is the first gate.
...
A model trained only on English gets these wrong everywhere else. Parallel labeled data across the seven languages provides the cross-lingual signal per-language classifiers need. The same generation pipeline extends to further languages as they are added.
