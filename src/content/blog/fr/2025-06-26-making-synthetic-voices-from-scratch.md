...
**2. Appliquer la conversion de voix.** Une étape de conversion de voix transforme le timbre de la donneuse en une nouvelle identité — genre, âge ou personnage différents. L'audio obtenu ressemble à la voix cible, pas à la donneuse. C'est là qu'une nouvelle personnalité voit le jour.

**3. Entraîner un modèle VITS compact.** L'audio converti devient le jeu d'entraînement d'un petit modèle d'architecture VITS via [phoonnx_train](https://github.com/TigreGotico/phoonnx). Le modèle terminé est exporté vers ONNX et fonctionne entièrement hors ligne — sur un Raspberry Pi si nécessaire.
