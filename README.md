...
website — a static Astro 5 site with zero client-side JavaScript on regular
pages, self-hosted fonts, and no third-party requests.
...
- **[phoonnx.js](https://github.com/TigreGotico/phoonnx.js)** +
  `onnxruntime-web` for the in-browser text-to-speech demo (`/demo` — the only
  page that ships JS)
...
pages, JSON for projects/datasets/models/testimonials. **See
...
Pages on push to `dev` or `master`, plus a weekly rebuild so date-scheduled
blog posts publish automatically. The site is currently served under
`/tigregotico-website/`; the `BASE` constant in `astro.config.mjs` documents
the move-to-root plan.
