...
important site also needs a mobile app. Not because HTML stopped working —
because an app is a *controlled surface*: a curated set of actions, no chrome
you didn't choose, an interface built for one way of interacting.
...
**Layer one — a typed client that turns a site into an API.** This is exactly
our [scraping and API-reverse-engineering work](/blog/2026-04-20-music-database-scrapers):
reach into a site that has no usable public interface and hand back structured,
typed objects instead of brittle HTML — verbs, not scraping:
...
site worth using is worth a *voice*. We are building them one at a time, in the
open, and every one of them makes the web a little more browsable for people the
