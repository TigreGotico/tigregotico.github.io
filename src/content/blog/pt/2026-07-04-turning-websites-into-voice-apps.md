...
conjunto curado de ações, sem chrome do navegador (menus, separadores e barras
de endereço) que não escolhemos, uma interface construída para uma única forma
de interagir.
...
**A camada um é um cliente tipado que transforma um site numa API.** Isto é
exatamente o nosso [trabalho de scraping e engenharia inversa de APIs](/pt/blog/2026-04-20-music-database-scrapers):
alcança um site que não tem qualquer interface pública utilizável e devolve
...
**A camada dois é um plugin OVOS que fala essa API.** Por cima do cliente assenta
...
Não é, deliberadamente, uma skill à medida por site, porque esse caminho leva a
...
Dá a um LLM um conjunto documentado de funções, como `search_albums`,
`get_recommendations` e `stream_url`, e ele traduzirá alegremente "encontra-me
...
que um site que vale a pena usar vale uma *voz*. Construímo-los um de cada
vez, de forma aberta, e cada um torna a web um pouco mais navegável para as
