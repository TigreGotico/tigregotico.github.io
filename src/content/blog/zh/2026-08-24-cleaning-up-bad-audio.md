...
在一次降噪或扩展前后各运行一次，一次真正的比较的形态就浮现出来了：用 DNSMOS 或 UTMOS 分别测原始音频和处理后的音频，看感知质量是否真的发生了变化；而在存在干净参考音频的情况下——合成噪声测试中有，但真实电话通话中很少有——用 SI-SDR 或 STOI 看处理后的信号是否真的向参考音频靠拢，而不只是听起来不一样了。这与上面降噪器表格中信噪比数字背后的原则是同一套：一个绑定了具体噪声条件的数字，而不是一个形容词。这套纯 ONNX 库家族更广泛的图景，包括 `speechonnxmetrics` 本身，参见 [一族纯 ONNX 语音库](/zh/blog/2026-08-03-a-family-of-pure-onnx-speech-libraries)。
