...
description: "Een getraind spraakmodel op een onderzoeks-GitHub-pagina is geen spraakassistent. Wij zetten open ASR- en TTS-checkpoints om naar ONNX, CoreML en GGUF, kwantiseren ze en valideren de uitvoer. Vervolgens publiceren we de resultaten onder OpenVoiceOS, zodat elke taal die ze dekken terechtkomt in een echte, offline assistent."
date: 2026-08-01
lang: nl
...
- **CoreML** is Apples formaat voor on-device-inferentie. Een CoreML-pakket draait op de Neural Engine of GPU van een Mac of iPhone in plaats van op de CPU. Dat is van belang voor realtime spraakherkenning op Apple-hardware.
...
We kunnen een exact cijfer plakken op de afweging voor één echt model. `nvidia/parakeet-tdt-0.6b-v3` is een ASR-model met 0,6 miljard parameters. De CoreML-mel-encoder-component is 1132,5 MB op volledige precisie. Gepalettiseerd (Apples term voor deze kwantisatiestap) naar 4 bits wordt dat 284,2 MB, een reductie van 3,99x, vrijwel exact overeenkomend over de drie subcomponenten (encoder, decoder, joint-decision-netwerk). Over het hele pakket is de niet-gekwantiseerde CoreML-export ongeveer 1,14 GB, en de 4-bit-versie ongeveer 293 MB. Dat is het verschil tussen een model dat comfortabel op een telefoon past en een model dat het maar net redt.
...
De reden is eenvoudig. OpenVoiceOS is waar de modellen worden gebruikt. Een geconverteerd model in een bedrijfsaccount is een aardig artefact. Datzelfde model gepubliceerd op de plek waar [`ovos-stt-plugin-onnx-asr`](https://github.com/OpenVoiceOS/ovos-stt-plugin-onnx-asr), [`ovos-stt-plugin-coreml`](https://github.com/TigreGotico/ovos-stt-plugin-coreml), of [`ovos-stt-plugin-rover`](https://github.com/TigreGotico/ovos-stt-plugin-rover) het op naam kunnen vinden, is een taal die een echte assistent nu kan spreken of begrijpen. Publiceren onder de eigen organisatie van het platform zet een conversie om in ondersteunde functionaliteit in plaats van een onderzoekscuriositeit. Het is hoe we ervoor zorgen dat dit werk eenmaal doen elke OpenVoiceOS-installatie ten goede komt, niet alleen de klant die erom vroeg.

Om duidelijk te zijn over toeschrijving: wij trainen deze akoestische modellen niet zelf vanaf nul, en we beweren dat ook niet. Het onderliggende onderzoek behoort toe aan de teams die het getraind hebben: NVIDIA's Parakeet- en Conformer-modellen, de IndicConformer-modellen van AI4Bharat voor Indiase talen, modellen van universiteiten en publieke instituten zoals het Galicische Proxecto Nós of de Conformer-modellen van het Baskische HiTZ-centrum, en onafhankelijke inspanningen om modellen voor Afrikaanse en minderheidstalen te converteren. Wat wij toevoegen is de conversie, de kwantisatie, de correctheidscontrole tegen het origineel, en de plugin-bekabeling waarmee een assistent het resultaat op naam kan laden.

De omvang van dat conversiewerk, rechtstreeks geteld uit wat is gepubliceerd, ziet er als volgt uit. Meer dan negentig Parakeet-ASR-varianten (over groottes, talen en kwantisatieniveaus) zijn geëxporteerd naar ONNX en CoreML. Er zijn meer dan dertig NVIDIA Conformer-modellen, tweeëntwintig AI4Bharat IndicConformer-modellen die talen met weinig hulpbronnen uit India dekken, en tweeëntwintig wav2vec2-modellen voor talen waaronder Zweeds, IJslands, Faroees, Fins en beide geschreven vormen van het Noors. Negen Conformer-modellen dekken Baskisch en Galicisch, en onafhankelijk geconverteerde Whisper- en wav2vec2-modellen dekken Afrikaanse en creoolse talen zoals Shona, Zoeloe, Xhosa, Malagasi, Haïtiaans Creools en Kabylisch. Als je alleen bevestigde ASR-conversies telt per afzonderlijke taalcode, zijn dat minstens 74 verschillende talen met vandaag een offline, gekwantiseerde spraakherkenner beschikbaar. Dat aantal telt de aparte catalogus van TTS-stemmen geëxporteerd voor talen zoals Baskisch, Aragonees, Asturisch, Galicisch, Occitaans en Arabisch niet mee.

## Als jouw taal of jouw apparaat vandaag niets offline heeft

De meeste talen krijgen nooit een commerciële offline spraakoptie, omdat de markt voor die ene taal een leverancier er niet toe brengt er een te bouwen. Het patroon hierboven hangt niet af van marktgrootte: een bestaand open checkpoint nemen, converteren naar een formaat dat draait op de hardware die je daadwerkelijk hebt, kwantiseren zodat het past, verifiëren tegen het origineel, en inbouwen in een plugin. Het hangt af van de aanwezigheid van een open checkpoint om mee te beginnen, en dat is steeds vaker het normale geval.
