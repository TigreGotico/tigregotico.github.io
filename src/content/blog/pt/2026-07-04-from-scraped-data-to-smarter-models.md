...
Uma pergunta justa é *porquê*. Somos uma empresa de IA de voz. Porque continuamos
a manter scrapers para enciclopédias de música e diretórios de rádio?
...
arquitetura. Vem dos dados, e os dados interessantes raramente estão
à espera num dataset pronto a usar. Estão espalhados pela web pública, em catálogos
que os humanos passaram décadas a curar.
...
- **Correspondência de intenção baseada em palavras-chave**: as listas de entidades
  tornam-se gazetteers (listas de nomes conhecidos) que ancoram as consultas de media
  no OpenVoiceOS.
...
- **Biasing por modelo de linguagem**: os LMs n-gram ou de shallow fusion (modelos de
  linguagem incorporados no passo de descodificação do reconhecedor) treinados
...
mais de 100.000 pares palavra→IPA de português europeu (IPA é o Alfabeto Fonético Internacional, uma forma padronizada de escrever a pronúncia). Esse dataset:
...
curadas, dicionários oficiais. É também por isso que cada dataset que publicamos indica
...
limpas sobre fontes desordenadas são mais do que uma conveniência de scraping. São a
