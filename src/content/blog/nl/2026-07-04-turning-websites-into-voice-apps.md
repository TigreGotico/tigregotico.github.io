...
acties, geen chrome die je niet koos, een interface gebouwd voor één manier van
interacteren.
...
**Laag één — een getypeerde client die een site in een API verandert.** Dit is
precies ons [scraping- en API-reverse-engineeringwerk](/nl/blog/2026-04-20-music-database-scrapers):
reik in een site die geen bruikbare publieke interface heeft en geef
...
**Laag twee — een OVOS-plugin die die API spreekt.** Boven op de client zit een
...
Het is bewust *geen* op maat gemaakte skill per site — die weg leidt tot tientallen
...
Geef een LLM een gedocumenteerde set functies — `search_albums`, `get_recommendations`,
`stream_url` — en het zal met plezier "vind me iets als Naxatras maar zwaarder"
