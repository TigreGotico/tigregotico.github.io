...
Der Datensatz verwendet vier Typen auf oberster Ebene, die direkt darauf abbilden, wie `little_questions` (die Inferenzbibliothek, die diese Daten konsumiert) Äußerungen routet:

- **question** — weiter unterteilt nach Untertyp: `yes_no_question`, `wh_question`, `tag_question`. Die EAT-Taxonomie innerhalb von `little_questions` fügt 53 feingranulare Antworttyp-Labels hinzu (Person, Ort, Menge, Definition und so weiter), aber die Satztyp-Klassifizierung ist das erste Tor.
- **command** — Imperativ- und Aufforderungsformen. Befehle erwarten keine Antwort; sie erwarten eine Aktion.
