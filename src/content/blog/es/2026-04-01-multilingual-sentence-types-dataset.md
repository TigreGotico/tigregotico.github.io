...
El dataset usa un conjunto plano de seis etiquetas: una columna `label` por fila, sin división tipo/subtipo. Estas etiquetas se corresponden directamente con cómo `little_questions` (la biblioteca de inferencia que consume estos datos) enruta las locuciones:

- **wh_question**: preguntas construidas en torno a una palabra interrogativa (qué, dónde, quién, etc.).
- **polar_question**: preguntas de sí/no. La taxonomía EAT (Expected Answer Type, tipo de respuesta esperado) dentro de `little_questions` añade 53 etiquetas de tipo de respuesta de grano fino (persona, ubicación, cantidad, definición, etc.) sobre las etiquetas de pregunta, pero la clasificación por tipo de oración es la primera puerta.
...
Un modelo entrenado solo en inglés se equivoca en todo lo demás. Los datos paralelos etiquetados a través de las siete lenguas proporcionan la señal translingüística que necesitan los clasificadores por lengua. La misma pipeline de generación se extiende a más lenguas a medida que se añaden.
