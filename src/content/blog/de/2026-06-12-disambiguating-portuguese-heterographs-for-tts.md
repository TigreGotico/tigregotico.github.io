...
Die POS-basierten Ansätze stagnieren genau dort, wo man es erwarten würde: Sie können nach Grammatik routen, aber niemals nach Bedeutung, sodass die Aufteilungen innerhalb der Substantivklasse außer Reichweite bleiben. Unser Resolver, die Bibliothek [`bifonia`](https://github.com/TigreGotico/bifonia), ist leichtgewichtig und **völlig abhängigkeitsfrei**. Er erreicht **≈94 %** und, was entscheidend ist, **100 %** im Fall `sede`/*Durst*, bei dem die POS-Tagger **0 %** erzielen.

Die Zahl zählt weniger als der Grund dahinter: Eine kleine, schnelle, vollständig offene Komponente schlägt schwergewichtige neuronale POS-Tagger bei dieser Aufgabe, weil sie die *Bedeutung* auflöst, nicht nur die Grammatik. Keine GPU, kein Modell-Download, kein Netzwerkaufruf.
...
Da der Datensatz offen und der Resolver winzig und forkbar ist, kann jede Person, die ein portugiesisches TTS-Front-End baut, dies ohne ein gigantisches Modell richtig hinbekommen. Derselbe Ansatz lässt sich sauber auf eine verwandte Sprache wie das Galicische übertragen, wo die Unterscheidung zwischen offenem und geschlossenem Vokal dieselbe Falle schafft. Die annotierten Daten erfüllen zudem einen doppelten Zweck: Sie sind genau das, was man braucht, um kompakte statistische Modelle zu trainieren, etwa einen Klassifikator pro Wort, für Teams, die über das Korpus verfügen und neben dem regelbasierten einen gelernten Resolver wünschen.
