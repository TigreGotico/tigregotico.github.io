...
acciones, sin cromo de navegador (menús, pestañas y barras de dirección) que no elegiste, una interfaz construida para una única forma de
interactuar.
...
**Capa uno es un cliente tipado que convierte un sitio en una API.** Esto es
exactamente nuestro [trabajo de scraping e ingeniería inversa de APIs](/es/blog/2026-04-20-music-database-scrapers):
alcanza un sitio que no tiene interfaz pública utilizable y devuelve objetos tipados
...
**Capa dos es un plugin de OVOS que habla esa API.** Sobre el cliente se asienta un
...
Deliberadamente *no* es una skill a medida por sitio, porque ese camino lleva a docenas de
...
Dale a un LLM un conjunto documentado de funciones, como `search_albums`,
`get_recommendations` y `stream_url`, y traducirá encantado "búscame algo como Naxatras
...
nuestra es que un sitio que vale la pena usar vale una *voz*. Los construimos
uno a uno, en abierto, y cada uno hace la web un poco más navegable para las
