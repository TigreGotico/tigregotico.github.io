...
A fair question is *why*. We are a voice-AI company. Why are we
...
architecture diagrams. It comes from data, and the interesting data
rarely sits in a ready-made dataset. It is spread across the public web, in
...
- **Keyword-based intent matching**: entity lists become gazetteers (lookup
  lists of known names) that ground media queries in OpenVoiceOS.
...
- **Language-model biasing**: n-gram or shallow-fusion LMs (language models
  blended into the recogniser's decoding step) trained on entity-rich text
  nudge the decoder toward in-domain words. A media
...
over 100,000 European Portuguese word→IPA pairs (IPA is the International Phonetic Alphabet, a standard way to write pronunciation). That dataset:
...
encyclopedias, official dictionaries. It is also why every dataset we publish states
...
API wants to ground its answers in. Clean APIs over messy sources are more than
